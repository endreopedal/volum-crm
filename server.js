...
app.post('/api/hent-leads', async (_req, res) => {
  try {
    ...
    const eks = await sbSelect('crm_leads?select=navn');
    const sett = new Set(eks.map(r => (r.navn || '').toLowerCase().trim()));
    ...
    for (const p of funnet) {
      ...
      nye.push({
        ...
        facebook: googleSok(navn, 'facebook'),
        instagram: googleSok(navn, 'instagram'),
        status: 'Ny',
        kilde: `places:${query}`
      });
    }
