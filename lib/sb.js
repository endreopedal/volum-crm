...
/**
 * Kjører en ren SELECT via ceo_readonly_query.
 *
 * Spørringen trimmes først. Vakten i databasen bruker btrim(), som bare
 * fjerner mellomrom — ikke linjeskift. En spørring som begynner med et
 * linjeskift består derfor ikke «starter med select»-testen, og avvises
 * med «Kun SELECT er tillatt» selv om den er en helt vanlig SELECT.
 */
const sbSql = (q) => sbRpc('ceo_readonly_query', { q: String(q).trim() });
