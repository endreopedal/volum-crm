// Leads-CRM: Google Places-henting, statusflyt og manuelle leads.
// Flyttet ut av server.js uendret da dashbordet kom til.
...
router.post('/', async (req, res) => {
  try {
    const { name, phone, website, fb, ig, bransje, by } = req.body;
    if (!name) return res.status(400).json({ feil: 'name mangler' });
    const rows = await sbInsert('crm_leads', [{
      name, phone, website, fb, ig, bransje, by,
...
router.patch('/:id', async (req, res) => {
  try {
    const patch = {};
    ['status', 'level', 'name', 'phone', 'website', 'fb', 'ig', 'note', 'next_contact', 'bransje', 'by']
...
router.post('/hent-leads', async (_req, res) => {
  try {
